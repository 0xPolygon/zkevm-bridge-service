package bridgectrl

import (
	"context"

	"github.com/0xPolygon/zkevm-bridge-service/etherman"
	"github.com/ethereum/go-ethereum/common"
)

// merkleTreeStore interface for the Merkle Tree
type merkleTreeStore interface {
	Get(ctx context.Context, key []byte, dbTx interface{}) ([][]byte, error)
	BulkSet(ctx context.Context, rows [][]interface{}, dbTx interface{}) error
	GetRoot(ctx context.Context, depositCount uint32, network uint32, dbTx interface{}) ([]byte, error)
	SetRoot(ctx context.Context, root []byte, depositID uint64, network uint32, dbTx interface{}) error
	GetLastDepositCount(ctx context.Context, networkID uint32, dbTx interface{}) (uint32, error)
	AddRollupExitLeaves(ctx context.Context, rows [][]interface{}, dbTx interface{}) error
	GetRollupExitLeavesByRoot(ctx context.Context, root common.Hash, dbTx interface{}) ([]etherman.RollupExitLeaf, error)
	GetLatestRollupExitLeaves(ctx context.Context, dbTx interface{}) ([]etherman.RollupExitLeaf, error)
	IsRollupExitRoot(ctx context.Context, root common.Hash, dbTx interface{}) (bool, error)
}
